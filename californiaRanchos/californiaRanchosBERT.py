...
# Load the HuggingFace BERT model for NER
...
def parseGrantsFile(file_path):
    ...
    for grant in grantsData:
        ...
        coordinates_match = re.search(r"In\s*T\s*[\d\-]+\s*[NS]?\s*,?\s*R\s*[\d\-]+\s*[EW]?\s*,?\s*(SBM|MDM|[\w\.]*)",
                                      grant)
...
def main():
    ...
    df.to_csv(os.path.join(outputDir, "landGrantHuggingFace.csv"), index=False)
    print(f"CSV file saved: {os.path.join(outputDir, 'landGrantHuggingFace.csv')}")
