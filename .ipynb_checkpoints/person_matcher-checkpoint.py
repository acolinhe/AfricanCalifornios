...
class PersonMatcher:
    ...
    def direct_match_names(self):
        compare_names = lambda x, y: match_names_score(x, y) if x.strip() and y.strip() else 0

        self.matched_records['First Name Match Classifier'] = \
        self.matched_records.apply(lambda row: compare_names(row['Census First Name'], row['Baptisms First Name']), axis=1)
        self.matched_records['Last Name Match Classifier'] = \
        self.matched_records.apply(lambda row: compare_names(row['Census Last Name'], row['Baptisms Last Name']), axis=1)
    ...
    def match_parents_names(self):
        compare_names = lambda x, y: match_names_score(str(x).strip(), str(y).strip()) if str(x).strip() and str(y).strip() else 0

        self.matched_records['Mother First Name Match Classifier'] = \
        self.matched_records.apply(lambda row: compare_names(row['Census First Name'], row['Mother Baptisms First Name']), axis=1)
        self.matched_records['Mother Last Name Match Classifier'] = \
        self.matched_records.apply(lambda row: compare_names(row['Census Last Name'], row['Mother Baptisms Last Name']), axis=1)
        self.matched_records['Father First Name Match Classifier'] = \
        self.matched_records.apply(lambda row: compare_names(row['Census First Name'], row['Father Baptisms First Name']), axis=1)
        self.matched_records['Father Last Name Match Classifier'] = \
        self.matched_records.apply(lambda row: compare_names(row['Census Last Name'], row['Father Baptisms Last Name']), axis=1)
